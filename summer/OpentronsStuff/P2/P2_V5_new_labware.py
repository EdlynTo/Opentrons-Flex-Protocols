...
metadata = {'protocolName': 'P2-iST V5'}
...
def run(ctx):
    ...
    tubes = ctx.load_labware("tuberack_eppendorf_12x2ml_falcon_6x15ml_conical", "C3", "tube rack")
    ...
    def tube_load_vols(vol_per_sample, cols_per_tube):
        ...
        # Each tube gets its own tube_dv plus a flat one-row (8-well) plate_dv buffer,
        # both added once per tube, so summed across tubes this equals:
        #   vol_per_sample * total_samples + (tube_dv + plate_dv*8) * num_tubes
        ...
        for _ in range(n_tubes):
            ...
            vols.append(vol_per_sample*samples_this_tube + tube_dv + plate_dv*8)
            cols_left -= cols_this_tube
        return vols

    def falcon_tube_load_vols(vol_per_sample):
        return vol_per_sample * COLUMNS * 8 + falcon_tube_dv + plate_dv*8

    bind = dw_res.rows()[0][0]
    bind_ = dw_res.columns()[0]
    bind_tube = tubes['A5']
    bind_tubes = [bind_tube]
    bind_vols = [falcon_tube_load_vols(bind_vol)]
    ...
    lyse_tube = tubes['B5']
    lyse_tubes = [lyse_tube]
    lyse_vols = [falcon_tube_load_vols(lyse_vol)]
    ...
    stop_tube = tubes['C5']
    stop_tubes = [stop_tube]
    stop_vols = [falcon_tube_load_vols(stop_vol)]
