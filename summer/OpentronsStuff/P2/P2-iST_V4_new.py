...
def run(ctx):
    ...
    # Labware
    sample_plate = hs_adapter.load_labware('thermofisher_96_wellplate_250ul', label='Sample Plate (Thermo 450)')
    ...
    # Tips
    tips_51 = ctx.load_labware('opentrons_flex_96_tiprack_50ul',location='A1',label='50ul Tips #1')
    tips_50 = [tips_51]
    ...
    def remove_supernatant(pip,vol,wells,remove=False, plate=None):
        ...
        if remove:
            ctx.move_labware(plate, hs_adapter, use_gripper=True)
            hs.close_labware_latch()
    ...
    for t, tube in enumerate(bead_tubes):
        ...
        for i in range(range1 if t == 0 else range2): # repeats for up to 6 columns worth of samples
            ...
            for well in bead_:
                single.aspirate(bead_vol+(25/COLUMNS), tube.bottom(height_))
                ctx.delay(seconds=1)
                single.dispense(single.current_volume, well.bottom().move(types.Point(x=radius-touch_off,y=0,z=2)))
                ctx.delay(seconds=1)
                height_ -= (bead_vol+(25/COLUMNS))/area # update new height after each aspirate/ dispense cycle
            ctx.comment('\n')
    drop(single)
    ...
    for t, tube in enumerate(bind_tubes):
        ...
        for i in range(range1 if t == 0 else range2 if t == 1 else range3 if t == 2 else range4): # repeats for up to 6 columns worth of samples
            for well in bind_:
                single.aspirate(bind_vol+(25/COLUMNS), tube.bottom(height_))
                ctx.delay(seconds=1)
                single.dispense(single.current_volume, well.bottom().move(types.Point(x=radius-touch_off,y=0,z=2)))
                ctx.delay(seconds=1)
                height_ -= (bind_vol+(25/COLUMNS))/area # update new height after each aspirate/ dispense cycle
            ctx.comment('\n')
    drop(single)
    ...
    for t, tube in enumerate(lyse_tubes):
        ...
        for i in range(range1 if t == 0 else range2 if t == 1 else range3 if t == 2 else range4): # repeats for up to 6 columns worth of samples
            for well in lyse_:
                single.aspirate(lyse_vol+(25/COLUMNS), tube.bottom(height_))
                ctx.delay(seconds=1)
                single.dispense(single.current_volume, well.bottom().move(types.Point(x=radius-touch_off,y=0,z=2)))
                ctx.delay(seconds=1)
                height_ -= (lyse_vol+(25/COLUMNS))/area # update new height after each aspirate/ dispense cycle
            ctx.comment('\n')
    drop(single)
    ...
    for t, tube in enumerate(stop_tubes):
        ...
        for i in range(range1 if t == 0 else range2 if t == 1 else range3 if t == 2 else range4): # repeats for up to 6 columns worth of samples
            for well in stop_:
                single.aspirate(stop_vol+(25/COLUMNS), tube.bottom(height_))
                ctx.delay(seconds=1)
                single.dispense(single.current_volume, well.bottom().move(types.Point(x=radius-touch_off,y=0,z=2)))
                ctx.delay(seconds=1)
                height_ -= (stop_vol+(25/COLUMNS))/area # update new height after each aspirate/ dispense cycle
            ctx.comment('\n')
    drop(single)
    ...
    ctx.move_labware(sample_plate if COLUMNS<=6 else int_plate, hs_adapter, use_gripper=True)
    hs.close_labware_latch()
